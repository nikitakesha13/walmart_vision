...
class Ui_nioshDialog(object):
        def __init__(self, _weight, _grip, _freq, _objDist, _hDist, _vDist, _gui):
                self.weight = _weight
                self.grip = _grip
                self.freq = _freq
        ...
        def getNIOSHIndex(self):
                ...
                self.calcObj = Calc(self.hDist, self.vDist, self.objDist, self.grip, self.weight)
                niosh_in = self.calcObj.liftingIndex()

                self.gui.setWeight(self.weight)
                self.gui.setGrip(self.grip)
                self.gui.setFrequency(self.freq)
                self.gui.setObjDist(self.objDist)
                self.gui.setHDist(self.hDist)
                self.gui.setVDist(self.vDist)

                self.gui.riskLabel.setText(str(niosh_in))
